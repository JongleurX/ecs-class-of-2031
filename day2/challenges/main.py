...
DISPLAY = {
    ...
    "dice_duel": ("🎲", "Dice Duel"),
    "mood_suggester": ("🌦️", "Mood Suggester"),
    "pizza_builder": ("🍕", "Pizza Builder"),
    "trivia_quiz": ("🧠", "Trivia Quiz"),
    "name_scrambler": ("🐍", "Name Scrambler"),
    "compliment_machine": ("🌈", "Compliment Machine"),
    "pixel_art": ("🖼️", "Pixel Art"),
}
...
def resolve_quick_run(items):
    ...
    print("Falling back to challenge menu.")
    return None
...
def show_menu(items):
    ...
    if not items:
        print("No Python challenge files found.")
        return
...
def main():
    while True:
        ...
        choice = input("Choose a challenge number: ").strip().lower()
